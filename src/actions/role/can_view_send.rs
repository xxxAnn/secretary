...
impl CanViewSend {
    ...
    pub fn action(self) -> VoteAction {
        VoteAction::CanViewSend(self)
    }
}

#[poise::command(
    slash_command,
    description_localized("en-US", "Allows role to view and send in a channel (default).")
)]
pub async fn can_view_send(
    ctx: Context<'_>,
    #[description = "Role to block from the channel"] role: serenity::Role,
    #[description = "Channel to block the role from speaking in"] channel: serenity::GuildChannel,
) -> Result<(), Error> {
    info!(
    ...
    if vec![1069130087116578908].contains(&role.id.0) {
        ...
        create_vote(
            &ctx,
            format!(
                "Allow role <@&{}> to view and speak in <#{}>",
