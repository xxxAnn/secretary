...
impl ChannelPurge {
    ...
    pub async fn call(self, http: impl AsRef<Http>) {
        ...
        let mids = cid.messages(&http, |ms| ms.limit(self.limit)).await.unwrap();
...
#[poise::command(slash_command, rename="purge", description_localized("en-US", "Deletes the last 100 messages in the channel."))]
pub async fn channel_purge(
    ctx: Context<'_>,
    #[description = "Channel in which to purge messages"]
    ...
    if vec![969016622402650112, 970108683746951178].contains(&channel.id.0) {
        let _ = ctx.send(|m| m.content("You cannot purge from these channels.").ephemeral(true)).await;
        Ok(())
    } else {
        create_vote(
            &ctx, 
