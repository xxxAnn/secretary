...
vote_action!(
    TextChannelCreate,
    move |me: TextChannelCreate, http: std::sync::Arc<Http>| {
        async move {
...
#[poise::command(slash_command, rename = "create_text")]
pub async fn create_text(
    ...
    #[description = "Channel topic"] topic: String,
) -> Result<(), Error> {
    create_vote(
        &ctx,
        format!("Create channel called {}", name),
        TextChannelCreate::new(name, category.map(|c| c.id().0), topic).action(),
    )
    .await
}
